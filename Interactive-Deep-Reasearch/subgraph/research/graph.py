...
def create_intelligent_initial_state(
    topic: str,
    section: Dict[str, Any],
    previous_sections_summary: List[str] = None,
    upcoming_sections_outline: List[str] = None,
    report_main_thread: str = "",
    ...
    return {
        ...
        "upcoming_sections_outline": upcoming_sections_outline or [],
        "report_main_thread": report_main_thread,
        "writing_style": writing_style,
        "logical_connections": {},

        # 研究阶段
        "initial_research_results": [],
        "supplementary_research_results": [],
...
from intelligent_section_research_graph import (
...
async def run_intelligent_research():
    config = {"configurable": {"thread_id": "intelligent_research_001"}}
    ...
    async for event in app.astream(initial_state, config=config, stream_mode="custom"):
        if isinstance(event, dict):
            ...
            if content and content.get("type") == "quality_assessment":
                ...
                print(f"  质量评分: {data.get('overall_quality', 0):.3f}")
                print(f"  内容缺口: {len(data.get('content_gaps', []))}个")
    ...
    if final_state and final_state.values:
        ...
        print(f"\\n🎉 智能研究完成！")
        print(f"最终质量评分: {final_result.get('quality_metrics', {}).get('final_quality_score', 0):.3f}")
        print(f"迭代次数: {final_result.get('quality_metrics', {}).get('iteration_count', 0)}")
        print(f"内容长度: {len(final_result.get('final_content', ''))}字符")
