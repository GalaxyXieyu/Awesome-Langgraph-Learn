...
import logging
import time
# from langgraph.config import get_stream_writer  # 兼容性问题，暂时注释

# 全局流写入器实例，由WorkflowAdapter设置
_global_stream_writer = None

def set_global_stream_writer(writer):
    """设置全局流写入器实例"""
    global _global_stream_writer
    _global_stream_writer = writer
    logging.info("✅ 全局流写入器已设置")

def get_stream_writer():
    """获取流写入器实例"""
    global _global_stream_writer
    if _global_stream_writer is not None:
        return _global_stream_writer
    else:
        # 降级到日志输出
        def fallback_writer(data):
            logging.info(f"Stream writer (fallback): {data}")
...
def rag_enhancement_node(state: WritingState) -> WritingState:
    ...
    try:
        writer = get_stream_writer()
        writer({"step": "rag_enhancement", "status": "询问RAG增强权限", "progress": 0})
    except Exception:
        writer = lambda _: None

    # 使用通用确认节点
    result_state = rag_confirmation_node(state)
    
    writer({"step": "rag_enhancement", "status": "RAG增强确认完成", "progress": 100})
    return result_state
    ...
    try:
        ...
        async for chunk in llm.astream(messages, config=config):
            if chunk.content and isinstance(chunk.content, str):
                ...
                # 每10个chunk发送一次进度更新
                if chunk_count % 10 == 0:
                    progress = min(90, 10 + (chunk_count // 10) * 5)  # 逐步增加到90%
                    writer({
...
def create_writing_assistant_graph(custom_stream_writer=None):
    ...
    try:
        ...
        # from langgraph.checkpoint.redis import RedisSaver  # 在0.0.62版本中不可用
        ...
        # 暂时使用内存checkpoint，等待LangGraph版本升级
        checkpointer = MemorySaver()
        logger.info("✅ 使用内存 checkpoint (Redis checkpoint在当前版本不可用)")

    except Exception as e:
        # 回退到内存 checkpoint（开发环境）
        checkpointer = MemorySaver()
        logger.warning(f"⚠️ 配置 checkpoint 失败，使用内存 checkpoint: {e}")

    # 编译图，支持checkpointer
    # 在LangGraph 0.0.62版本中，compile函数的参数结构不同
    app = workflow.compile(
        checkpointer=checkpointer,
        interrupt_before=None,  # 可以在这里配置需要中断的节点
        interrupt_after=None,   # 可以在这里配置需要中断的节点
        debug=False
    )

    # 注意：custom_stream_writer在当前版本中可能不支持
    if custom_stream_writer:
        logger.info("⚠️ 自定义流式写入器在当前LangGraph版本中可能不支持")
